...
onMounted(async () => {
  ...
  try {
    ...
    if (contact.value.email && contact.value.email == contact.value.emailPending) {
      isLinear.value = false
      await nextTick()
      step.value = 2
    } else if (contact.value.emailPending) {
      step.value = 1
    }
  } catch (error) {
...
const emailvalidation =
  /^[a-zA-Z0-9._%+-]+@((xn--)?[a-zA-Z0-9\u00A0-\uFFFF-]+(\.[a-zA-Z0-9\u00A0-\uFFFF-]+)*\.[a-zA-Z\u00A0-\uFFFF]{2,})$/
const complete = computed(
  () =>
    !!contact.value.name &&
    emailvalidation.test(contact.value.emailPending) &&
    !!contact.value.phonePending
)
const emailValidated = computed(
  () => !!contact.value.email && contact.value.email == contact.value.emailPending
)
...
const signup = async (next) => {
  ...
  const pendingEmail = contact.value.emailPending
  const pendingPhone = contact.value.phonePending
  const pendingName = contact.value.name
  try {
    const body = JSON.stringify({
      teamId: contact.value.teamId,
      type: props.teamType,
      emailPending: pendingEmail,
      phonePending: pendingPhone,
      name: pendingName
    })
    ...
    contact.value = {
      ...team,
      emailPending: team.emailPending || pendingEmail,
      phonePending: team.phonePending || pendingPhone,
      name: team.name || pendingName
    }
    next()
    if (team.teamId) {
      router.replace({ path: '/indskrivning/' + team.teamId })
    }
  } catch (error) {
...
const validatePincode = async () => {
  ...
  try {
    ...
    const response = await fetch('/api/signup/pincode', {
      method: 'POST',
      body: body,
      headers: headers
    })
    ...
    if (data.ok) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Fejl',
        detail: 'Validering af telefonnummer fejlede: forkert pin',
        life: 5000
      })
    ...
    <Stepper :linear="isLinear" v-model:activeStep="step">
      ...
      <StepperPanel header="Bekræft e-mailadresse">
        <template #content="{ prevCallback, nextCallback }">
          <div class="flex flex-col h-[12rem]">
            <p>
              For at sikre at vi kan komme i kontakt med jer er det vigtigt at den indtastede
              e-mailadresse er korrekt. Vi har sendt en e-mail med et bekræftigelseslink til:
            </p>
            <div class="text-center py-5">
              <p class="text-xl font-bold">{{ contact.emailPending }}</p>
            </div>
          ...
          <div class="flex pt-4 justify-between">
            ...
            <Button
              ...
              iconPos="right"
              @click="nextCallback"
              :disabled="contact.email != contact.emailPending"
            />
          </div>
        </template>
      </StepperPanel>
      <StepperPanel header="Bekræft telefonnummer">
        <template #content="{ prevCallback }">
          <div class="flex flex-col h-[12rem]">
            <p>
              For at bekræfte dit telefonnummer har vi sendt en SMS med en pinkode - indtast
              pinkoden herunder
            </p>
          ...
          <div class="flex pt-4 justify-between">
            ...
            <Button
              ...
              :disabled="!emailValidated || !contact.pincode || contact.pincode.length < 4"
            />
          </div>
        </template>
      </StepperPanel>
