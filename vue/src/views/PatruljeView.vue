...
const expenses = computed(() => {
  ...
  return new List(
    ...
    { "text": "Års t-shirt", "count": tshirtCount, "unitPrice": config.value.tshirtPrice, "amount": tshirtCount*config.value.tshirtPrice }
  );
})
...
onMounted(async () => {
  try {
    const response = await fetch("/patrulje-123.json");
    if (!response.ok) {
...
const saveMember = () => {
    ...
    if (member.value.id) {
        ...
        member.value.code = createId();
        member.value.image = 'product-placeholder.svg';
        //member.value.inventoryStatus = product.value.inventoryStatus ? product.value.inventoryStatus.value : 'INSTOCK';
...
<Fieldset class="mt-3" legend="Spejdere">
    <div class="card">
        <DataTable :value="members" size="small" tableStyle="min-width: 50rem">
            ...
            <Column field="tshirt" header="T-Shirt">
                <template #body="row" style="font-size:0.8rem">
                    {{ row.data.tshirtcount }} &times; {{ row.data.tshirtsize }}
                </template>
            </Column>
        ...
        <Dialog v-model:visible="paymentDialog" :style="{width: '500px'}" header="Betaling" :modal="true">
            <div class="confirmation-content">
                ...
                <InputGroup size="small">
                    ...
                    <InputText size="small" placeholder="Telefonnummer" :value="contact.phone" />
                </InputGroup>
