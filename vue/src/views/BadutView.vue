...
const load = async (userId) => {
  try {
    ...
    // assignment above has settled. See CrewView.vue for the full
    // rationale — in short, priming inside the watcher silently
    // swallows the user's first selection when the loaded tshirtSize
    // equals the initial '' (no value change → watcher never fires).
    await nextTick()
...
// recomputed when the t-shirt selection changes (the only "product"
// add/remove flow available in this single-user view).
...
// only acts after load() has explicitly primed it via nextTick — see
// CrewView.vue for the rationale.
...
// The size this gøgler actually has on an order. While the sale is closed this is
// what the page shows, rather than staffer.tshirtSize: an unpaid selection has been
// cancelled off the open order, and the stored size would keep promising a shirt
// nobody will produce.
    ...
    <div class="grid grid-cols-2 gap-4">
      <Fieldset class="mt-3" legend="Kontaktinformation">
        ...
        <div class="flex flex-col">
          <FloatLabel class="mt-7 select-none">
            <InputText
              id="h"
              v-model.trim="staffer.phone"
              size="small"
              class="w-full select-none"
              required="true"
              :invalid="teamSubmitted && !team.phone"
            />
      ...
      <Fieldset class="mt-3" legend="Gøgler">
        <div v-for="q in questions" class="flex flex-col gap-2 pb-3">
          ...
          >
            <RadioButton
              v-model="staffer.additionals[q.slug]"
              :inputId="q.slug + i"
              :name="q.slug"
              :value="o"
              size="small"
