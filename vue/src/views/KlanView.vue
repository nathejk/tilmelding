...
// Expenses, payments, totals are derived from the server-side order
...
// that tell the team their new size was registered. See PRD 002.
...
// showPaymentsSection gates the entire Betalinger fieldset — in addition
// to the existing initialSignup gate, this also requires either a
// current bill or any historical paid orders.
...
const requestSeat = async () => {
  ...
  try {
    const payload = JSON.stringify({
      ...
      teamGroup: team.value.group,
      teamCorps: team.value.korps,
      requestedMemberCount: Math.floor(team.value.memberCount)
    })
    const response = await fetch('/api/klan/' + props.teamId + '/request', {
      method: 'PUT',
      body: payload,
      headers: headers
    })
...
// memberTshirtLabel is what a senior's t-shirt cell or dialog should read.
//
  ...
  <div class="container mx-auto">
    <div class="grid grid-cols-2 gap-4">
      <Fieldset class="mt-3" legend="Klanoplysninger">
        <p class="m-0">
          Klanen skal bestå af mellem {{ config.minMemberCount }} og
          {{ config.maxMemberCount }} banditter.
        </p>
        ...
        <div class="flex flex-col" v-if="initialSignup">
          ...
          <div class="flex flex-wrap gap-3">
            <div class="flex items-center">
              <RadioButton
                v-model="team.memberCount"
                inputId="memberCount-1"
                name="memberCount"
                value="1"
              />
    ...
    <!--
      Informational here: per-senior sizes are set in the member dialog, not
    ...
    -->
    <Shop v-if="!initialSignup" :open="tshirtOpen" />
          ...
          <template #footer>
            <div class="text-end">
              <Button
                ...
                rounded
                @click="openNew"
                :disabled="members.length >= config.maxMemberCount"
              />
            </div>
          </template>
        </DataTable>
    ...
    <Fieldset class="mt-3" legend="Betalinger" v-if="!initialSignup && showPaymentsSection">
      <div class="card">
